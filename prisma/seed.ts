...
async function main() {
  ...
  console.log('⭐ Creant testimonis...');

  const testimonials = [
    {
      authorName: 'M.G.',
      authorLabel: 'Boda en Granollers',
      rating: 5,
      eventType: 'WEDDING',
      isFeatured: true,
      order: 1,
      translations: {
        es: { quote: 'La mejor decisión que tomamos para nuestra boda. La pista estuvo llena toda la noche y los invitados no paraban de decirnos lo bien que se lo pasaron.' },
        ca: { quote: 'La millor decisió que vam prendre per al nostre casament. La pista va estar plena tota la nit i els convidats no paraven de dir-nos el bé que s\'ho van passar.' },
        en: { quote: 'The best decision we made for our wedding. The dance floor was packed all night and guests kept telling us how much fun they had.' },
      },
    },
    {
      authorName: 'P.L.',
      authorLabel: 'Cumpleaños 40',
      rating: 5,
      eventType: 'BIRTHDAY',
      isFeatured: true,
      order: 2,
      translations: {
        es: { quote: 'Profesionales de verdad. Desde el primer contacto hasta el final del evento, todo perfecto. Los efectos de CO2 fueron espectaculares.' },
        ca: { quote: 'Professionals de veritat. Des del primer contacte fins al final de l\'event, tot perfecte. Els efectes de CO2 van ser espectaculars.' },
        en: { quote: 'True professionals. From the first contact to the end of the event, everything was perfect. The CO2 effects were spectacular.' },
      },
    },
    {
      authorName: 'Corporate',
      authorLabel: 'Evento empresa Barcelona',
      rating: 5,
      eventType: 'CORPORATE',
      isFeatured: true,
      order: 3,
      translations: {
        es: { quote: 'Contratamos a Òrbita para nuestra fiesta de empresa y superaron todas las expectativas. Muy profesionales y el equipo técnico de primera.' },
        ca: { quote: 'Vam contractar Òrbita per la nostra festa d\'empresa i van superar totes les expectatives. Molt professionals i l\'equip tècnic de primera.' },
        en: { quote: 'We hired Òrbita for our company party and they exceeded all expectations. Very professional and top-notch technical equipment.' },
      },
    },
  ];
  ...
  for (let i = 0; i < testimonials.length; i++) {
    ...
    const created = await prisma.testimonial.upsert({
      ...
      create: {
        ...
        rating: t.rating,
        eventType: t.eventType as any,
        isFeatured: t.isFeatured,
        order: t.order,
      },
    });

    for (const [locale, data] of Object.entries(t.translations)) {
      await prisma.testimonialTranslation.upsert({
        where: { testimonialId_locale: { testimonialId: created.id, locale } },
        update: {},
        create: { testimonialId: created.id, locale, ...data },
      });
    }
  }

  console.log(`   ✅ ${testimonials.length} testimonis creats`);

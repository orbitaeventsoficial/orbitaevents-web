#!/usr/bin/env node
/**
 * Guard: zero rgba literals hardcoded als components i pàgines de l'admin.
 *
 * Escàner de fitxers .tsx dins app/admin/ que detecta rgba(NNN,... literals.
 ...
 * No detecta expressions computades com rgba(${r}, ${g}, ${b}, 0.22).
 *
...
// Fitxers amb rgba legítim per raons tècniques específiques
...
// Detecta rgba( seguit d'un dígit numèric — literal, no expressió computada
...
for (const file of walkTsx(ADMIN_DIR)) {
  ...
  for (let i = 0; i < lines.length; i++) {
    const line = lines[i];
    const matches = [...line.matchAll(RGBA_RE)];
...
if (violations.length === 0) {
  console.log('✓ check-inline-rgba: cap rgba literal inline als components admin.');
  process.exit(0);
}

console.error('✗ check-inline-rgba: rgba literals hardcoded detectats als components admin:\n');
for (const v of violations) {
  console.error(`  ${v.file}:${v.line}:${v.col}  ${v.rgba}`);
  console.error(`    → ${v.text}`);
}
console.error(`\nTotal: ${violations.length} violació(ns).`);
console.error('Moure els colors a ADMIN_SVG_COLORS o ADMIN_CHART_SERIES_COLORS');
console.error("a lib/constants/admin.ts.");
process.exit(1);

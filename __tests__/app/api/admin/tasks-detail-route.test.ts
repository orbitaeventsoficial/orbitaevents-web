import { NextRequest } from 'next/server';
import { beforeEach, describe, expect, it, vi } from 'vitest';

const { mockRequireAuth, mockUpdateTask, mockDeleteTask } = vi.hoisted(() => ({
  mockRequireAuth: vi.fn(),
  mockUpdateTask: vi.fn(),
  mockDeleteTask: vi.fn(),
}));

vi.mock('@/lib/auth', () => ({
  requireAuth: mockRequireAuth,
}));
vi.mock('@/lib/services/tasks/taskAdminService', () => ({
  updateAdminTask: mockUpdateTask,
  deleteAdminTask: mockDeleteTask,
}));
vi.mock('@/lib/logger', () => ({
  log: { error: vi.fn(), info: vi.fn(), warn: vi.fn() },
}));

import { PATCH, DELETE } from '@/app/api/admin/tasks/[id]/route';

function makePatchReq(id: string, body: Record<string, unknown>) {
  return {
    req: new NextRequest(`http://localhost/api/admin/tasks/${id}`, {
      method: 'PATCH',
      body: JSON.stringify(body),
      headers: { 'Content-Type': 'application/json' },
    }),
    params: { params: { id } },
  };
}

function makeDeleteReq(id = 'task-1') {
  return {
    req: new NextRequest(`http://localhost/api/admin/tasks/${id}`, { method: 'DELETE' }),
    params: { params: { id } },
  };
}

describe('PATCH /api/admin/tasks/[id]', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mockRequireAuth.mockReturnValue(null);
    mockUpdateTask.mockResolvedValue({ ok: true, task: { id: 'task-1', status: 'DONE' } });
  });

  it('rebutja sense autenticació', async () => {
    mockRequireAuth.mockReturnValueOnce(
      new Response(JSON.stringify({ error: 'Unauthorized' }), { status: 401 }),
    );
    const { req, params } = makePatchReq('task-1', { status: 'DONE' });
    const res = await PATCH(req, params);
    expect(res.status).toBe(401);
  });

  it('actualitza tasca correctament', async () => {
    const { req, params } = makePatchReq('task-1', { status: 'DONE' });
    const res = await PATCH(req, params);
    expect(res.status).toBe(200);
    expect(mockUpdateTask).toHaveBeenCalledWith('task-1', { status: 'DONE' });
  });

  it('actualitza múltiples camps', async () => {
    const { req, params } = makePatchReq('task-1', { title: 'Nou títol', priority: 'URGENT', assignedTo: 'admin' });
    const res = await PATCH(req, params);
    expect(res.status).toBe(200);
    expect(mockUpdateTask).toHaveBeenCalledWith('task-1', { title: 'Nou títol', priority: 'URGENT', assignedTo: 'admin' });
  });

  it('rebutja status invàlid', async () => {
    const { req, params } = makePatchReq('task-1', { status: 'INVALID' });
    const res = await PATCH(req, params);
    expect(res.status).toBe(400);
  });

  it('rebutja priority invàlida', async () => {
    const { req, params } = makePatchReq('task-1', { priority: 'EXTREME' });
    const res = await PATCH(req, params);
    expect(res.status).toBe(400);
  });

  it('rebutja title buit', async () => {
    const { req, params } = makePatchReq('task-1', { title: '' });
    const res = await PATCH(req, params);
    expect(res.status).toBe(400);
  });

  it('retorna 500 si falla', async () => {
    mockUpdateTask.mockRejectedValueOnce(new Error('DB'));
    const { req, params } = makePatchReq('task-1', { status: 'DONE' });
    const res = await PATCH(req, params);
    expect(res.status).toBe(500);
  });
});

describe('DELETE /api/admin/tasks/[id]', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mockRequireAuth.mockReturnValue(null);
    mockDeleteTask.mockResolvedValue({ ok: true });
  });

  it('rebutja sense autenticació', async () => {
    mockRequireAuth.mockReturnValueOnce(
      new Response(JSON.stringify({ error: 'Unauthorized' }), { status: 401 }),
    );
    const { req, params } = makeDeleteReq();
    const res = await DELETE(req, params);
    expect(res.status).toBe(401);
  });

  it('elimina tasca correctament', async () => {
    const { req, params } = makeDeleteReq('task-1');
    const res = await DELETE(req, params);
    expect(res.status).toBe(200);
    expect(mockDeleteTask).toHaveBeenCalledWith('task-1');
  });

  it('retorna 500 si falla', async () => {
    mockDeleteTask.mockRejectedValueOnce(new Error('DB'));
    const { req, params } = makeDeleteReq();
    const res = await DELETE(req, params);
    expect(res.status).toBe(500);
  });
});

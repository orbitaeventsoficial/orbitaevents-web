...
<tr><td style="padding:24px 32px;border-top:1px solid rgba(255,255,255,0.06);background:rgba(255,255,255,0.02);"><div style="font-size:13px;color:rgba(255,255,255,0.5);">Gràcies per confiar en nosaltres.</div><div style="margin-top:8px;font-size:12px;color:rgba(255,255,255,0.3);">📞 ${EMAIL_CONTACT.phone} · ✉ ${EMAIL_CONTACT.email} · 🌐 ${SITE_CONFIG.web.domain}</div><div style="margin-top:12px;font-size:11px;color:rgba(255,255,255,0.2);">© ${new Date().getFullYear()} Òrbita Events · Granollers, Barcelona</div></td></tr>
</table>
</td></tr>
</table>
</body></html>`;
...
function getDefaultTemplate(slug: TemplateSlug, locale: string): { subject: string; bodyHtml: string } {
  const templates: Record<string, Record<string, { subject: string; bodyHtml: string }>> = {
    ...
    testimonial_reminder: {
      ...
      en: { subject: '{{clientName}}, we\'d love your review! ⭐', bodyHtml: emailShell(`<div style="font-size:26px;font-weight:800;color:#ffffff;">Hi {{clientName}}!</div><div style="margin-top:12px;font-size:15px;color:rgba(255,255,255,0.6);line-height:1.7;">A few days ago we shared a party together and your review would really help us improve. It takes less than a minute!</div>${ctaButton('Leave a review ⭐', '{{reviewUrl}}')}`) },
    },
    welcome: {

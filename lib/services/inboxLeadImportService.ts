...
export async function importLeadFromInboxMessage(uidNum: number, fallbackEmail: FallbackEmailPayload = {}) {
  if (!Number.isFinite(uidNum)) {
    return { status: 400, body: { error: 'UID invàlid' } };
  }
  ...
  if (!senderAddress || !senderAddress.includes('@')) {
    return { status: 400, body: { error: 'Email remitent invàlid' } };
  }
  ...
  const extracted = {
    ...
    eventSchedule: sanitizeString(extractedRaw.eventSchedule, 180),
  ...
  if (!isValidEmail(extracted.email)) {
    return { status: 400, body: { error: 'No s’ha pogut detectar un email vàlid del remitent' } };
  }
  ...
  if (existing) {
    ...
    const updated = await prisma.lead.update({
      ...
      data: {
        ...
        eventSchedule: existing.eventSchedule || extracted.eventSchedule || null,
  ...
  const created = await prisma.lead.create({
    data: {
      ...
      eventSchedule: extracted.eventSchedule || null,

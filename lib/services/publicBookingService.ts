...
export async function createPublicBooking(body: PublicBookingRequest) {
  ...
  if (!pack) {
    return { status: 400, body: { success: false, error: 'Invalid pack selected' } };
  }
  ...
  if (body.extraIds && body.extraIds.length > 0) {
    ...
    if (extras.length !== body.extraIds.length) {
      return { status: 400, body: { success: false, error: 'One or more extras are invalid.' } };
    }
  }

...
type Variant = 'nit' | 'marfil' | 'joia' | 'contrast';
...
/* Aspectes commutables (defineixen color i contrast de tota l'app) */
...
function VariantSwitch({ v, set }: { v: Variant; set: (x: Variant) => void }) {
  return (
    <div className="fx-switch" role="group" aria-label="Aspecte">
      <span className="fx-switch__lab">Aspecte</span>
      {VARIANTS.map((o) => (
        <button key={o.id} type="button" className={v === o.id ? 'is-on' : ''} aria-pressed={v === o.id} onClick={() => set(o.id)}>{o.label}</button>
      ))}
    </div>
  );
}
...
function TemporadaPage({ viewMode, setViewMode, months, onOpen }: {
    ...
    <>
      <div className="fx__pagehead">
        <div className="fx__tt">
          <span className="fx__eyebrow">Comercial · Entrades</span>
          <h1 className="fx__h1">Temporada</h1>
        </div>
        <div className="fx__headright">
          ...
          <span className="fx__sub">{MONTHS_SHORT[MONTHS[0] - 1]} – {MONTHS_SHORT[MONTHS[MONTHS.length - 1] - 1]} {YEAR} · caps de setmana</span>
        </div>
      </div>
      <div className="fx__hint"><span className="fx__hintdot" /> Pipeline per treballar el lead amb tota la informació. Calendari per veure l&apos;ocupació dels caps de setmana.</div>
          ...
          <div className="fx__cal">
            {months.map((month) => (
              <article className="fx__mon" key={month.m}>
                <div className="fx__monhead"><h2>{month.label}</h2></div>
                <div className="fx__grid">
                  ...
                  {month.weekends.map((w) => w.days.map((d) => {
                    const l = d.lead;
                    if (!l) return <span key={d.iso} className={`fx__cell is-free${d.inMonth ? '' : ' is-out'}`}><span className="fx__day">{d.d}</span></span>;
                    return (
                      <button key={d.iso} type="button" className={`fx__cell is-lead${d.inMonth ? '' : ' is-out'}`} data-stage={l.stage} onClick={() => onOpen(l.id)}>
                        <span className="fx__celltop">
                          <span className="fx__day">{d.d}</span>
                          <span className="fx__cellmeta"><span className="fx__wx" data-wx={l.wx.kind}>{WX_ICON[l.wx.kind]}</span><span className="fx__stage">{STAGE_LABEL[l.stage]}</span></span>
                        </span>
              ...
              <section className="fx__lane" key={stage} data-stage={stage}>
                ...
                <div className="fx__lanecards">
                  {LEADS.filter((l) => l.stage === stage).map((l) => (
                    <button key={l.id} type="button" className="fx__pipelead" data-stage={l.stage} onClick={() => onOpen(l.id)}>
                      <span className="fx__stage">{STAGE_LABEL[l.stage]}</span>
                      <b>{l.name}</b>
                      <span>{fullDate(l.dateISO)} · {l.time}</span>
                      <span>{l.pax} pax · {l.product}</span>
...
export default function LeadsLabPage() {
  const [variant, setVariant] = useState<Variant>('contrast');
  ...
  return (
    <main className={`fx-root is-${variant}`}>
      <VariantSwitch v={variant} set={setVariant} />
      <AppShell>
        {pageLead
          ? <LeadPage lead={pageLead} onBack={() => setPageId(null)} />
          : <TemporadaPage viewMode={viewMode} setViewMode={setViewMode} months={months} onOpen={setPageId} />}
      </AppShell>
    </main>
  );
}

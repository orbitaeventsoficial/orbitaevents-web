...
export default function GalleryPasswordGate({ shareToken, locale, wrongPassword }: Props) {
  ...
  return (
    <div className="w-full max-w-sm">
      <div className="rounded-2xl border border-white/10 bg-white/[0.04] px-8 py-10 text-center">
        <p className="text-xs uppercase tracking-[0.2em] text-cyan-400 mb-2">Orbita Events</p>
        <h1 className="text-xl font-bold mb-1">Galeria protegida</h1>
        <p className="text-sm text-white/50 mb-6">Introdueix la contrasenya per veure les fotos</p>

        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label htmlFor="gallery-password" className="sr-only">Contrasenya</label>
            <input
              id="gallery-password"
              type="password"
              value={password}
          ...
          {wrongPassword && (
            <p className="text-sm text-red-400">Contrasenya incorrecta. Torna-ho a intentar.</p>
          )}

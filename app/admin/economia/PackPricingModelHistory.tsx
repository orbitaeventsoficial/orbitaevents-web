...
export default function PackPricingModelHistory({ entries }: { entries: PackHistoryEntry[] }) {
  ...
  return (
    <section className="rounded-2xl border border-white/10 bg-slate-950/60 p-5 shadow-sm">
      <h2 className="text-lg font-semibold text-slate-100">Històric model econòmic packs</h2>
      <div className="mt-3 grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <label className="text-xs text-slate-400">
          Des de
          <input type="date" value={dateFrom} onChange={(e) => setDateFrom(e.target.value)} className="mt-1 w-full rounded-md border border-stone-300 bg-slate-950/60 px-2 py-1.5 text-xs text-slate-200" />
        </label>
        <label className="text-xs text-slate-400">
          Fins a
          <input type="date" value={dateTo} onChange={(e) => setDateTo(e.target.value)} className="mt-1 w-full rounded-md border border-stone-300 bg-slate-950/60 px-2 py-1.5 text-xs text-slate-200" />
        </label>
        <label className="text-xs text-slate-400">
          Cercar canvis
          <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="marge, operari, especialista..." className="mt-1 w-full rounded-md border border-stone-300 bg-slate-950/60 px-2 py-1.5 text-xs text-slate-200" />
        </label>
      </div>

      <div className="mt-3 space-y-2">
        {filteredEntries.length === 0 ? (
          <p className="text-sm text-slate-400">Encara no hi ha versions desades del model de packs.</p>
        ) : (
          filteredEntries.map((entry) => (
            <div key={entry.id} className="rounded-lg border border-white/10 p-3">
              <div className="flex flex-wrap items-center justify-between gap-2">
                <p className="text-sm font-semibold text-slate-100">{new Date(entry.createdAt).toLocaleString('ca-ES')}</p>
                <button
                  type="button"

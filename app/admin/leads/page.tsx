  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      ...
      <header className="flex items-center justify-between">
        <div>
          <h1 className="text-xl sm:text-2xl font-semibold tracking-tight text-stone-800">Leads</h1>
        ...
        <Link
          href="/admin"
          className="inline-flex items-center rounded-xl border border-amber-200 bg-white px-3 py-2 text-xs sm:text-sm font-medium text-stone-600 hover:bg-amber-50 active:bg-amber-100 transition-colors"
        >
          ← Tornar
        </Link>
      </header>
      ...
      <section className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        <div className="rounded-2xl border border-amber-100 bg-white p-3 sm:p-5 shadow-sm">
          <p className="text-[10px] sm:text-xs font-medium text-stone-500 uppercase">Total</p>
          <p className="mt-1 sm:mt-2 text-2xl sm:text-3xl font-bold text-stone-800">{stats.total}</p>
        </div>
        <div className="rounded-2xl border border-sky-200 bg-gradient-to-br from-sky-50 to-blue-50 p-3 sm:p-5 shadow-sm">
          <p className="text-[10px] sm:text-xs font-medium text-sky-600 uppercase">Nous</p>
          <p className="mt-1 sm:mt-2 text-2xl sm:text-3xl font-bold text-sky-700">{stats.nous}</p>
        </div>
        <div className="rounded-2xl border border-amber-200 bg-gradient-to-br from-amber-50 to-orange-50 p-3 sm:p-5 shadow-sm">
          <p className="text-[10px] sm:text-xs font-medium text-amber-600 uppercase">Negociació</p>
          <p className="mt-1 sm:mt-2 text-2xl sm:text-3xl font-bold text-amber-700">{stats.enNegociacio}</p>
        </div>
        <div className="rounded-2xl border border-emerald-200 bg-gradient-to-br from-emerald-50 to-green-50 p-3 sm:p-5 shadow-sm">
          <p className="text-[10px] sm:text-xs font-medium text-emerald-600 uppercase">Convertits</p>
          <p className="mt-1 sm:mt-2 text-2xl sm:text-3xl font-bold text-emerald-700">{stats.convertits}</p>
        </div>
      </section>
      ...
      <section className="lg:hidden space-y-3">
        {leads.length === 0 ? (
          <div className="rounded-2xl border border-amber-100 bg-white p-8 text-center">
            ...
            <p className="mt-2 text-stone-500">Encara no hi ha leads</p>
            <p className="text-xs text-stone-400">Els contactes apareixeran aquí</p>
          </div>
        ) : (
          leads.map((lead) => {
            ...
            return (
              <Link
                ...
                className="block rounded-2xl border border-amber-100 bg-white p-4 shadow-sm hover:bg-amber-50/50 active:bg-amber-100/50 transition-colors"
              >
                <div className="flex items-start justify-between gap-3">
                  <div className="flex items-center gap-3 min-w-0 flex-1">
                    <div className="w-10 h-10 rounded-full bg-gradient-to-br from-orange-100 to-amber-100 flex items-center justify-center text-orange-600 font-semibold shrink-0">
                ...
                {lead.booking && (
                  <div className="mt-2 text-xs text-emerald-600 font-medium">✓ Reserva: {lead.booking.reference}</div>
                )}
              </Link>
            );
          })
          ...
          <table className="w-full text-sm">
            <thead className="bg-gradient-to-r from-amber-50/50 to-transparent border-b border-amber-100">
              <tr>
                ...
                <th className="px-4 py-3 text-left font-medium text-stone-600">Tipus</th>
                  ...
                  return (
                    <tr key={lead.id} className="hover:bg-amber-50/50 transition-colors">
                      <td className="px-4 py-3">
                        <Link href={`/admin/leads/${lead.id}`} className="font-medium text-stone-700 hover:text-orange-600">
                          {lead.name}
                        </Link>
                        {lead.booking && (
                          <div className="text-xs text-emerald-600">✓ {lead.booking.reference}</div>
                        )}
                      </td>
                      <td className="px-4 py-3">
                        <a href={`mailto:${lead.email}`} className="text-sky-600 hover:underline text-xs truncate block max-w-[180px]">
                          {lead.email}
                        </a>
                        {lead.phone && (
                          <a href={`tel:${lead.phone}`} className="text-stone-500 text-xs">📱 {lead.phone}</a>
                        )}
                      </td>

    ...
    <>
      ...
      <AnimatePresence>
        {moreMenuOpen && (
          ...
          >
            <motion.div
              className="absolute bottom-20 right-4 bg-zinc-900 rounded-2xl p-2 border border-white/10 shadow-2xl min-w-[200px]"
            ...
            >
              {moreMenuItems.map((item) => (
                <Link
                  ...
                  className="flex items-center gap-3 px-4 py-3 rounded-xl text-white/80 hover:bg-white/5 transition-colors"
                >
      ...
      >
        {/* Background */}
        <div className="absolute inset-0 bg-zinc-950/95 backdrop-blur-xl border-t border-white/5" />
          ...
          {navItems.map((item) => {
            ...
            if (item.id === 'more') {
              return (
                ...
                >
                  <motion.div
                    className={`relative ${moreMenuOpen ? 'text-orange-400' : 'text-white/60'}`}
                    animate={{ rotate: moreMenuOpen ? 90 : 0 }}
                  >
                    <Icon active={moreMenuOpen} />
                  </motion.div>
                  <span className={`text-[10px] mt-1 font-medium ${moreMenuOpen ? 'text-orange-400' : 'text-white/50'}`}>
                    {item.label}
                  </span>
                </button>
              );
            ...
            return (
              ...
              >
                ...
                {/* Icon with badge */}
                <motion.div
                  className={`relative ${isActive ? 'text-orange-400' : 'text-white/60'}`}
                  animate={{
                    y: isActive ? -2 : 0,
                ...
                {/* Label */}
                <span className={`text-[10px] mt-1 font-medium ${isActive ? 'text-orange-400' : 'text-white/50'}`}>
                  {item.label}
                </span>
              </Link>
            );

...
function MetricCard({ 
  ...
  return (
    ...
    >
      <Link
        href={href}
        className={`block p-4 rounded-2xl bg-gradient-to-br ${color} border border-stone-200 active:scale-[0.98] transition-transform relative overflow-hidden`}
      >
        {/* Glow effect */}
...
function MiniBarChart({ data, label }: { data: number[]; label: string }) {
  ...
  return (
    <motion.div
      className="p-4 bg-stone-50/[0.03] rounded-2xl border border-stone-200"
  ...
  return (
    ...
    >
      <Link
        ...
        className="flex items-center gap-3 p-3 bg-stone-50/[0.03] rounded-xl border border-stone-200 active:bg-stone-50/[0.06] transition-colors"
      >
        ...
        {actions.map((action, i) => (
          ...
          >
            {action.external ? (
              <a
                href={action.href}
                className={`flex flex-col items-center gap-2 p-4 rounded-2xl bg-gradient-to-br ${action.color} border border-stone-200 min-w-[80px] active:scale-95 transition-transform`}
              >
...
export default function MobileDashboardPro() {
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen bg-stone-100 p-4 pt-6 flex items-center justify-center">
        <motion.div
  ...
  if (error && !data) {
    return (
      <div className="min-h-screen bg-stone-100 p-4 pt-6 flex flex-col items-center justify-center gap-4">

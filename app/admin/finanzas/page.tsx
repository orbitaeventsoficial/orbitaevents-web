  ...
  return (
    <div className="space-y-6">
      <header className="rounded-2xl border border-slate-700/60 bg-slate-900/70 p-6 shadow-sm">
        <h1 className="text-2xl font-semibold text-slate-100">Finances i cobraments</h1>
        <p className="mt-1 text-sm text-slate-300">
          Control de bestreta, saldos pendents i caixa prevista.
        </p>
      </header>

      <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <div className="rounded-xl border border-slate-700/60 bg-slate-900/70 p-4 shadow-sm">
          <p className="text-xs text-slate-400">Pendent total</p>
          <p className="text-2xl font-semibold text-slate-100">{currency(outstandingTotal)}</p>
        </div>
        <div className="rounded-xl border border-rose-500/30 bg-rose-500/10 p-4 shadow-sm">
          <p className="text-xs text-rose-300">Vençut</p>
          <p className="text-2xl font-semibold text-rose-200">{currency(overdueTotal)}</p>
        </div>
        <div className="rounded-xl border border-amber-500/30 bg-amber-500/10 p-4 shadow-sm">
          <p className="text-xs text-amber-300">Venç en 7 dies</p>
          <p className="text-2xl font-semibold text-amber-200">{currency(dueSoonTotal)}</p>
        </div>
        <div className="rounded-xl border border-emerald-500/30 bg-emerald-500/10 p-4 shadow-sm">
          <p className="text-xs text-emerald-300">Cobrat aquest mes</p>
          <p className="text-2xl font-semibold text-emerald-200">{currency(monthCollected)}</p>
        </div>
      </section>

      <section className="rounded-2xl border border-slate-700/60 bg-slate-900/70 p-5 shadow-sm">
        <div className="flex items-center justify-between">
          <h2 className="text-lg font-semibold text-slate-100">Cobraments vençuts</h2>
          <Link href="/admin/bookings" className="text-sm text-slate-300 hover:text-slate-100">
            Veure reserves
          </Link>
        </div>
        <div className="mt-3 space-y-2">
          {atRiskRows.length === 0 ? (
            <p className="text-sm text-slate-300">No hi ha cobraments vençuts.</p>
          ) : (
            atRiskRows.map((row) => (
              <div key={row.id} className="rounded-lg border border-slate-700/60 bg-slate-900/40 p-3">
                {(() => {
                  const paymentFlow = deriveFlowStatus(logsByBooking[row.id] || [], 'PAYMENT');
                  return (
                    <p className="mb-2 text-xs text-slate-300">
                      Estat contacte cobrament: <strong>{paymentFlow.state}</strong>
                    </p>
                  );
                })()}
                ...
                <p className="mt-1 text-xs text-slate-300">
                  Esdeveniment: {row.eventDate.toLocaleDateString('ca-ES')} · Estat: {row.status}
                </p>
                <div className="mt-2 grid gap-2 sm:grid-cols-2">
                  <div className="rounded-md border border-slate-700/60 bg-slate-900/60 p-2 text-xs">
                    <p className="text-slate-300">Bestreta ({currency(row.depositAmount)}) · venç {row.depositDueAt.toLocaleDateString('ca-ES')}</p>
                    <PaymentToggleButton bookingId={row.id} field="depositPaid" currentValue={row.depositPaid} />
                  </div>
                  <div className="rounded-md border border-slate-700/60 bg-slate-900/60 p-2 text-xs">
                    <p className="text-slate-300">Saldo ({currency(row.remainingAmount)}) · venç {row.remainingDueAt.toLocaleDateString('ca-ES')}</p>
      ...
      <section className="rounded-2xl border border-slate-700/60 bg-slate-900/70 p-5 shadow-sm">
        <h2 className="text-lg font-semibold text-slate-100">Pròxims cobraments (7 dies)</h2>
        <div className="mt-3 space-y-2">
          {upcomingDueRows.length === 0 ? (
            <p className="text-sm text-slate-300">No hi ha cobraments pròxims aquesta setmana.</p>
          ) : (
            upcomingDueRows.map((row) => (
              <div key={row.id} className="rounded-lg border border-slate-700/60 bg-slate-900/40 p-3 text-xs text-slate-300">
                {row.reference} · {row.clientName} · esdeveniment {row.eventDate.toLocaleDateString('ca-ES')}
              </div>
            ))
          )}
        </div>

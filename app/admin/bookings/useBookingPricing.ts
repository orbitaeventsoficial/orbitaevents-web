...
import { getMarginBand, getMarginLabel } from '@/lib/margin-utils';
import { aggregateServiceLines, classifyBoloLines, computeDirectCostBreakdown } from '@/lib/services/costEngine';
import {
...
export function useBookingPricing({ form, packs, extras, selectedExtras, customPackPrice, manualTotalPrice, invoiceRequired = false, serviceLines = [], internalTravelCostOverride }: UseBookingPricingOptions) {
  ...
  const marginEstimate = useMemo(() => {
    ...
    const { hasOwnEquipment } = classifyBoloLines(serviceLines);
    const hasExternalProviderService = serviceLines.some((line) => line.kind === 'PROVIDER_SERVICE' && Boolean(line.collaboratorId));
    const marginProfile = !selectedPack && !hasOwnEquipment && hasExternalProviderService ? 'external' : 'own';
    // Semàfor de marge canònic (4 bandes, font única getMarginBand): Excel·lent/Acceptable/Vigilar/Crític.
    const band = getMarginBand(marginPct, marginProfile);
    const tone: 'emerald' | 'amber' | 'orange' | 'rose' =
      band === 'excellent' ? 'emerald' : band === 'acceptable' ? 'amber' : band === 'watch' ? 'orange' : 'rose';
    return { directCost, netMargin, marginPct, tone, label: getMarginLabel(marginPct, marginProfile) };
  }, [internalTravelCost, pricing, selectedPack, serviceLines]);

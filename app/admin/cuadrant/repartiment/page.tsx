  ...
  return (
    ...
    >
      ...
      <div className="mb-4 flex items-center justify-center gap-3">
        <Link href={`/admin/cuadrant/repartiment?month=${prev}`} className="rounded-lg border border-white/10 bg-white/[0.03] px-3 py-1 text-sm hover:bg-white/[0.06]">←</Link>
        <span className="min-w-[140px] text-center text-sm font-semibold capitalize">{label}</span>
        <Link href={`/admin/cuadrant/repartiment?month=${next}`} className="rounded-lg border border-white/10 bg-white/[0.03] px-3 py-1 text-sm hover:bg-white/[0.06]">→</Link>
      </div>

      {/* Totals */}
      <div className="mb-5 grid grid-cols-1 gap-2 sm:grid-cols-3">
        <div className="rounded-xl border border-white/10 bg-white/[0.03] p-3 text-center">
          <p className="text-[9px] font-semibold uppercase tracking-wider opacity-50">Facturat (client)</p>
        ...
        <div className="rounded-xl border border-amber-500/25 bg-amber-500/[0.06] p-3 text-center">
          <p className="text-[9px] font-semibold uppercase tracking-wider opacity-50">A col·laboradors</p>
        ...
        <div className="rounded-xl border border-emerald-500/25 bg-emerald-500/[0.06] p-3 text-center">
          <p className="text-[9px] font-semibold uppercase tracking-wider opacity-50">La teva part</p>
          ...
          {payout.people.map((person) => (
            <section
              ...
              className={`rounded-xl border p-3 ${
                person.isOwner ? 'border-emerald-500/25 bg-emerald-500/[0.04]' : 'border-white/10 bg-white/[0.03]'
              }`}
            >
              ...
              {person.events.length > 0 && (
                <ul className="mt-2 space-y-0.5">
                  {person.events.map((e, i) => (
                    <li key={i} className="flex items-center justify-between text-[11px] opacity-60">

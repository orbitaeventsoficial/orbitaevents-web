// app/components/ui/HeaderPro.tsx
// ÒRBITA EVENTS - Header PROFESSIONAL v6
// Logo gran, elegant, sense rosa, 100% professional
...
/**
 * ═══════════════════════════════════════════════════════════════════
 * HEADER ÒRBITA EVENTS - Versió PROFESSIONAL v6
 * ═══════════════════════════════════════════════════════════════════
 * 
 * CARACTERÍSTIQUES:
 * - Logo GRAN i prominent
 * - Colors corporatius: DAURAT (ambre/or) + Negre
 * - SENSE rosa/pink/purple
 * - Navegació clara i professional
 * - Responsive perfecte
 */
    ...
    <>
      ...
      <header
        className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${
          isScrolled
            ? 'bg-black/95 backdrop-blur-xl shadow-2xl shadow-black/50 border-b border-white/5'
      ...
      >
        <div className="container mx-auto px-4 lg:px-8">
          <div className="flex items-center justify-between h-20 lg:h-24">
            ...
            >
              {/* Glow daurat subtil al hover */}
              <div className="absolute -inset-4 bg-gradient-to-r from-amber-500/10 to-orange-500/10 rounded-2xl blur-xl opacity-0 group-hover:opacity-100 transition-opacity duration-700" />
              <Image
                src="/img/logoplanetatextdreta.svg"
                alt="Òrbita Events"
                width={220}
                height={60}
                className="h-12 lg:h-14 xl:h-16 w-auto transition-all duration-300 group-hover:brightness-110 relative z-10"
                priority
              />
            </Link>
                      ...
                      <div className="p-2">
                        {services.map((service) => (
                          <Link
                            key={service.href}
                            href={service.href}
                            className="flex items-start gap-4 p-3 rounded-xl transition-all group hover:bg-amber-500/10"
                          >
                            ...
                            <svg
                              className="w-5 h-5 text-white/20 group-hover:text-amber-400 mt-1 transition-colors"
                              fill="none"
                              viewBox="0 0 24 24"
                              stroke="currentColor"
                            >
                ...
                <AnimatePresence>
                  {activeDropdown === 'experiences' && (
                    <motion.div
                      ...
                      exit={{ opacity: 0, y: 10, scale: 0.95 }}
                      transition={{ duration: 0.2, ease: [0.23, 1, 0.32, 1] }}
                      className="absolute top-full left-0 mt-2 w-96 bg-zinc-900/98 backdrop-blur-xl border border-white/10 rounded-2xl shadow-2xl shadow-black/50 overflow-hidden"
                    >
                      {/* Header del dropdown */}
                      ...
                      <div className="p-3">
                        {experiences.map((exp) => (
                          <Link
                            key={exp.href}
                            href={exp.href}
                            className="flex items-center gap-4 p-3 rounded-xl transition-all group hover:bg-amber-500/10"
                          >
                            {/* Icona */}
                            ...
                            <div className="flex-1">
                              ...
                              <span className="text-white/50 text-sm leading-tight block mt-0.5">
                                {t(exp.descKey)}
                              </span>
                            </div>
            ...
            {/* ═══════════════════════════════════════════════════════════ */}
            {/* HAMBURGER MOBILE - Gestionat per MobileHeader               */}
            {/* ═══════════════════════════════════════════════════════════ */}
            <div className="lg:hidden">
              {/* El botó hamburger està al MobileHeader */}
            </div>

// app/components/ui/HeaderPremium.tsx
// ÒRBITA EVENTS - Header BRUTAL v5
// INCLOU: Mon Magic + Halloween + Experiències Temàtiques completes

'use client';

import { useState, useEffect, useCallback } from 'react';
import Link from 'next/link';
import Image from 'next/image';
import { motion, AnimatePresence } from 'framer-motion';
import { useTranslations } from 'next-intl';
import LanguageSelector from './LanguageSelector';

/**
 * ═══════════════════════════════════════════════════════════════════
 * HEADER ÒRBITA EVENTS - Versió BRUTAL v5
 * ═══════════════════════════════════════════════════════════════════
 *
 * ESTRUCTURA:
 * LOGO | Serveis▼ | Experiències▼ | Portfolio | Contacte | [CTA]
 *
 * NOVETATS v5:
 * - Dropdown separat per Experiències Temàtiques (Mon Magic + Halloween)
 * - Animacions cinematogràfiques
 * - Mega menu amb previsualització d'imatges
 * - Efectes hover BRUTALS
 */

// ═══════════════════════════════════════════════════════════════════
// CONFIGURACIÓ DE SERVEIS
// ═══════════════════════════════════════════════════════════════════
const services = [
  {
    href: '/servicios/bodas',
    icon: '💍',
    titleKey: 'bodes',
    descKey: 'bodesDesc',
    gradient: 'from-pink-500/20 to-rose-500/20',
  },
  {
    href: '/servicios/fiestas',
    icon: '🎉',
    titleKey: 'festes',
    descKey: 'festesDesc',
    gradient: 'from-amber-500/20 to-orange-500/20',
  },
  {
    href: '/servicios/empresas',
    icon: '💼',
    titleKey: 'empreses',
    descKey: 'empresesDesc',
    gradient: 'from-blue-500/20 to-cyan-500/20',
  },
  {
    href: '/servicios/discomovil',
    icon: '🎵',
    titleKey: 'discomovil',
    descKey: 'discomovilDesc',
    gradient: 'from-purple-500/20 to-pink-500/20',
  },
];

// ═══════════════════════════════════════════════════════════════════
// CONFIGURACIÓ D'EXPERIÈNCIES TEMÀTIQUES - HALLOWEEN INCLÒS!
// ═══════════════════════════════════════════════════════════════════
const experiences = [
  {
    href: '/tematica-mon-magic',
    icon: '🪄',
    titleKey: 'monMagic',
    descKey: 'monMagicDesc',
    gradient: 'from-purple-600/30 to-blue-600/30',
    image: '/images/tematicas/mon-magic/hero/01-taula-panoramica-cartell.jpg',
    badgeKey: 'badges.popular',
    badgeColor: 'bg-purple-500',
  },
  {
    href: '/tematica-halloween',
    icon: '🎃',
    titleKey: 'halloween',
    descKey: 'halloweenDesc',
    gradient: 'from-orange-600/30 to-red-600/30',
    image: '/img/portfolio/fiestas-tematicas-halloween/fiestas-tematicas-halloween-01.jpg',
    badgeKey: 'badges.terrific',
    badgeColor: 'bg-orange-500',
  },
  {
    href: '/boda-halloween',
    icon: '💀',
    titleKey: 'bodaHalloween',
    descKey: 'bodaHalloweenDesc',
    gradient: 'from-gray-600/30 to-purple-600/30',
    image: '/img/portfolio/fiestas-tematicas-halloween/fiestas-tematicas-halloween-01.jpg',
  },
];

export function HeaderPremium() {
  const t = useTranslations('header');
  const tCommon = useTranslations('common');
  const tWhatsapp = useTranslations('whatsappMessages');
  const tAccessibility = useTranslations('accessibility');

  const [isScrolled, setIsScrolled] = useState(false);
  const [isMobileMenuOpen, setIsMobileMenuOpen] = useState(false);
  const [activeDropdown, setActiveDropdown] = useState<'services' | 'experiences' | null>(null);

  // Detectar scroll per canviar fons
  useEffect(() => {
    const handleScroll = () => {
      setIsScrolled(window.scrollY > 20);
    };
    window.addEventListener('scroll', handleScroll, { passive: true });
    return () => window.removeEventListener('scroll', handleScroll);
  }, []);

  // Tancar menú mobile al canviar de pàgina
  useEffect(() => {
    setIsMobileMenuOpen(false);
    setActiveDropdown(null);
  }, []);

  // Handlers per dropdowns
  const handleMouseEnter = useCallback((dropdown: 'services' | 'experiences') => {
    setActiveDropdown(dropdown);
  }, []);

  const handleMouseLeave = useCallback(() => {
    setActiveDropdown(null);
  }, []);

  return (
    <>
      {/* ═══════════════════════════════════════════════════════════════ */}
      {/* HEADER PRINCIPAL                                                */}
      {/* ═══════════════════════════════════════════════════════════════ */}
      <header
        className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${
          isScrolled
            ? 'bg-black/95 backdrop-blur-xl shadow-2xl shadow-black/50'
            : 'bg-gradient-to-b from-black/90 via-black/50 to-transparent'
        }`}
      >
        <div className="container mx-auto px-4 lg:px-6">
          <div className="flex items-center justify-between h-16 lg:h-20">

            {/* ═══════════════════════════════════════════════════════════ */}
            {/* LOGO AMB EFECTE GLOW                                        */}
            {/* ═══════════════════════════════════════════════════════════ */}
            <Link
              href="/"
              className="flex-shrink-0 group relative"
            >
              <div className="absolute -inset-2 bg-gradient-to-r from-amber-500/20 to-orange-500/20 rounded-full blur-lg opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
              <Image
                src="/img/logoplanetatextdreta.svg"
                alt="Òrbita Events"
                width={140}
                height={40}
                className="h-8 lg:h-10 w-auto transition-all duration-300 group-hover:scale-105 relative z-10"
                priority
              />
            </Link>

            {/* ═══════════════════════════════════════════════════════════ */}
            {/* NAV DESKTOP                                                 */}
            {/* ═══════════════════════════════════════════════════════════ */}
            <nav className="hidden lg:flex items-center gap-1">

              {/* SERVEIS (Desplegable) */}
              <div
                className="relative"
                onMouseEnter={() => handleMouseEnter('services')}
                onMouseLeave={handleMouseLeave}
              >
                <button
                  className={`px-4 py-2.5 text-sm font-medium rounded-xl transition-all flex items-center gap-2 ${
                    activeDropdown === 'services'
                      ? 'text-white bg-white/10'
                      : 'text-white/80 hover:text-white hover:bg-white/5'
                  }`}
                  aria-expanded={activeDropdown === 'services'}
                  aria-haspopup="true"
                >
                  {t('serveis')}
                  <motion.svg
                    className="w-4 h-4"
                    fill="none"
                    viewBox="0 0 24 24"
                    stroke="currentColor"
                    animate={{ rotate: activeDropdown === 'services' ? 180 : 0 }}
                    transition={{ duration: 0.2 }}
                  >
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
                  </motion.svg>
                </button>

                {/* Dropdown Serveis */}
                <AnimatePresence>
                  {activeDropdown === 'services' && (
                    <motion.div
                      initial={{ opacity: 0, y: 10, scale: 0.95 }}
                      animate={{ opacity: 1, y: 0, scale: 1 }}
                      exit={{ opacity: 0, y: 10, scale: 0.95 }}
                      transition={{ duration: 0.2, ease: [0.23, 1, 0.32, 1] }}
                      className="absolute top-full left-0 mt-2 w-80 bg-zinc-900/98 backdrop-blur-xl border border-white/10 rounded-2xl shadow-2xl shadow-black/50 overflow-hidden"
                    >
                      <div className="p-2">
                        {services.map((service) => (
                          <Link
                            key={service.href}
                            href={service.href}
                            className={`flex items-start gap-4 p-3 rounded-xl transition-all group hover:bg-gradient-to-r ${service.gradient}`}
                          >
                            <span className="text-2xl mt-0.5 group-hover:scale-125 transition-transform duration-300">
                              {service.icon}
                            </span>
                            <div className="flex-1">
                              <span className="text-white font-semibold block group-hover:text-amber-400 transition-colors">
                                {t(service.titleKey)}
                              </span>
                              <span className="text-white/50 text-sm leading-tight">
                                {t(service.descKey)}
                              </span>
                            </div>
                            <motion.svg
                              className="w-5 h-5 text-white/30 group-hover:text-amber-400 mt-1"
                              fill="none"
                              viewBox="0 0 24 24"
                              stroke="currentColor"
                              initial={{ x: 0 }}
                              whileHover={{ x: 4 }}
                            >
                              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
                            </motion.svg>
                          </Link>
                        ))}
                      </div>
                    </motion.div>
                  )}
                </AnimatePresence>
              </div>

              {/* ═══════════════════════════════════════════════════════════ */}
              {/* EXPERIÈNCIES TEMÀTIQUES (Mega Menu BRUTAL)                  */}
              {/* ═══════════════════════════════════════════════════════════ */}
              <div
                className="relative"
                onMouseEnter={() => handleMouseEnter('experiences')}
                onMouseLeave={handleMouseLeave}
              >
                <button
                  className={`px-4 py-2.5 text-sm font-medium rounded-xl transition-all flex items-center gap-2 ${
                    activeDropdown === 'experiences'
                      ? 'text-white bg-gradient-to-r from-purple-500/20 to-pink-500/20'
                      : 'text-white/80 hover:text-white hover:bg-white/5'
                  }`}
                  aria-expanded={activeDropdown === 'experiences'}
                  aria-haspopup="true"
                >
                  <span className="relative">
                    {t('experiencies')}
                    {/* Badge "Nou" */}
                    <span className="absolute -top-1 -right-8 px-1.5 py-0.5 text-[10px] font-bold bg-gradient-to-r from-purple-500 to-pink-500 text-white rounded-full">
                      ✨
                    </span>
                  </span>
                  <motion.svg
                    className="w-4 h-4 ml-4"
                    fill="none"
                    viewBox="0 0 24 24"
                    stroke="currentColor"
                    animate={{ rotate: activeDropdown === 'experiences' ? 180 : 0 }}
                    transition={{ duration: 0.2 }}
                  >
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
                  </motion.svg>
                </button>

                {/* MEGA MENU Experiències */}
                <AnimatePresence>
                  {activeDropdown === 'experiences' && (
                    <motion.div
                      initial={{ opacity: 0, y: 10, scale: 0.95 }}
                      animate={{ opacity: 1, y: 0, scale: 1 }}
                      exit={{ opacity: 0, y: 10, scale: 0.95 }}
                      transition={{ duration: 0.25, ease: [0.23, 1, 0.32, 1] }}
                      className="absolute top-full left-1/2 -translate-x-1/2 mt-2 w-[500px] bg-zinc-900/98 backdrop-blur-xl border border-white/10 rounded-3xl shadow-2xl shadow-purple-500/20 overflow-hidden"
                    >
                      {/* Header del mega menu */}
                      <div className="px-6 py-4 bg-gradient-to-r from-purple-600/20 to-pink-600/20 border-b border-white/10">
                        <h3 className="text-lg font-bold text-white flex items-center gap-2">
                          <span className="text-2xl">✨</span>
                          {t('experienciesTitle')}
                        </h3>
                        <p className="text-white/60 text-sm mt-1">
                          {t('experienciesSubtitle')}
                        </p>
                      </div>

                      {/* Grid d'experiències */}
                      <div className="p-4 grid grid-cols-1 gap-3">
                        {experiences.map((exp) => (
                          <Link
                            key={exp.href}
                            href={exp.href}
                            className={`relative flex items-center gap-4 p-4 rounded-2xl transition-all group overflow-hidden hover:bg-gradient-to-r ${exp.gradient}`}
                          >
                            {/* Imatge de fons amb overlay */}
                            {exp.image && (
                              <div className="absolute inset-0 opacity-0 group-hover:opacity-20 transition-opacity duration-500">
                                <Image
                                  src={exp.image}
                                  alt=""
                                  fill
                                  className="object-cover"
                                />
                              </div>
                            )}

                            {/* Icona */}
                            <div className="relative z-10 w-14 h-14 rounded-xl bg-white/10 flex items-center justify-center group-hover:scale-110 transition-transform duration-300">
                              <span className="text-3xl">{exp.icon}</span>
                            </div>

                            {/* Contingut */}
                            <div className="relative z-10 flex-1">
                              <div className="flex items-center gap-2">
                                <span className="text-white font-bold group-hover:text-amber-400 transition-colors">
                                  {t(exp.titleKey)}
                                </span>
                                {exp.badgeKey && (
                                  <span className={`px-2 py-0.5 text-[10px] font-bold ${exp.badgeColor} text-white rounded-full`}>
                                    {t(exp.badgeKey)}
                                  </span>
                                )}
                              </div>
                              <span className="text-white/50 text-sm leading-tight block mt-1">
                                {t(exp.descKey)}
                              </span>
                            </div>

                            {/* Fletxa */}
                            <motion.div
                              className="relative z-10 w-8 h-8 rounded-full bg-white/10 flex items-center justify-center group-hover:bg-amber-500 transition-colors"
                              whileHover={{ scale: 1.1 }}
                            >
                              <svg className="w-4 h-4 text-white/50 group-hover:text-black" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
                              </svg>
                            </motion.div>
                          </Link>
                        ))}
                      </div>

                      {/* Footer del mega menu */}
                      <div className="px-6 py-4 bg-white/5 border-t border-white/10">
                        <Link
                          href="/experiencias"
                          className="flex items-center justify-center gap-2 text-sm text-amber-400 hover:text-amber-300 font-medium transition-colors"
                        >
                          {t('viewAllExperiences')}
                          <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 8l4 4m0 0l-4 4m4-4H3" />
                          </svg>
                        </Link>
                      </div>
                    </motion.div>
                  )}
                </AnimatePresence>
              </div>

              {/* PORTFOLIO */}
              <Link
                href="/portfolio"
                className="px-4 py-2.5 text-sm font-medium text-white/80 hover:text-white hover:bg-white/5 rounded-xl transition-all"
              >
                {tCommon('nav.portfolio')}
              </Link>

              {/* CONTACTE */}
              <Link
                href="/contacto"
                className="px-4 py-2.5 text-sm font-medium text-white/80 hover:text-white hover:bg-white/5 rounded-xl transition-all"
              >
                {tCommon('nav.contact')}
              </Link>

            </nav>

            {/* ═══════════════════════════════════════════════════════════ */}
            {/* CTA DESKTOP                                                 */}
            {/* ═══════════════════════════════════════════════════════════ */}
            <div className="hidden lg:flex items-center gap-3">
              {/* Language Selector */}
              <LanguageSelector />

              {/* WhatsApp icon con pulse */}
              <a
                href={`https://wa.me/34699121023?text=${encodeURIComponent(tWhatsapp('general'))}`}
                target="_blank"
                rel="noopener noreferrer"
                className="relative p-2.5 text-white/50 hover:text-green-400 hover:bg-green-400/10 rounded-xl transition-all group"
                title="WhatsApp"
              >
                {/* Pulse indicator */}
                <span className="absolute top-1 right-1 w-2 h-2 bg-green-500 rounded-full animate-pulse" />
                <svg className="w-5 h-5 group-hover:scale-110 transition-transform" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/>
                </svg>
              </a>

              {/* Botó CTA principal BRUTAL */}
              <Link
                href="/contacto"
                className="relative px-6 py-2.5 overflow-hidden rounded-full group"
              >
                {/* Gradient background */}
                <div className="absolute inset-0 bg-gradient-to-r from-amber-500 to-orange-500 transition-all duration-300 group-hover:scale-105" />
                {/* Shine effect */}
                <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/25 to-transparent -translate-x-full group-hover:translate-x-full transition-transform duration-700" />
                {/* Text */}
                <span className="relative z-10 text-black font-bold text-sm">
                  {t('cta')}
                </span>
              </Link>
            </div>

            {/* ═══════════════════════════════════════════════════════════ */}
            {/* HAMBURGER MOBILE                                            */}
            {/* ═══════════════════════════════════════════════════════════ */}
            <button
              onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)}
              className="lg:hidden p-2 text-white hover:bg-white/10 rounded-xl transition-colors"
              aria-label={isMobileMenuOpen ? tAccessibility('closeMenu') : tAccessibility('openMenu')}
              aria-expanded={isMobileMenuOpen}
            >
              <motion.div
                className="w-6 h-6 flex flex-col justify-center items-center gap-1.5"
                animate={isMobileMenuOpen ? "open" : "closed"}
              >
                <motion.span
                  className="w-6 h-0.5 bg-white rounded-full block"
                  variants={{
                    closed: { rotate: 0, y: 0 },
                    open: { rotate: 45, y: 8 },
                  }}
                />
                <motion.span
                  className="w-6 h-0.5 bg-white rounded-full block"
                  variants={{
                    closed: { opacity: 1 },
                    open: { opacity: 0 },
                  }}
                />
                <motion.span
                  className="w-6 h-0.5 bg-white rounded-full block"
                  variants={{
                    closed: { rotate: 0, y: 0 },
                    open: { rotate: -45, y: -8 },
                  }}
                />
              </motion.div>
            </button>

          </div>
        </div>
      </header>

      {/* ═══════════════════════════════════════════════════════════════════ */}
      {/* MOBILE MENU - Es gestiona a MobileHeader.tsx                        */}
      {/* ═══════════════════════════════════════════════════════════════════ */}

      {/* ═══════════════════════════════════════════════════════════════════ */}
      {/* SPACER (per evitar que contingut quedi sota header fix)            */}
      {/* ═══════════════════════════════════════════════════════════════════ */}
      <div className="h-16 lg:h-20" aria-hidden="true" />
    </>
  );
}

export default HeaderPremium;

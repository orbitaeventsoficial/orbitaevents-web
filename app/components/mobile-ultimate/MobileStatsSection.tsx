...
function StatCard({ stat, delay, started }: StatCardProps) {
  const reduceMotion = useReducedMotion();
  const count = useCountUp(stat.value, 1800, started && !reduceMotion);
  ...
  return (
    <motion.div
      ...
      style={{
        background: `radial-gradient(ellipse at 50% 0%, ${stat.glow}, transparent 70%), rgba(255,255,255,0.04)`,
      }}
    >
      {/* Shine sweep */}
      {!reduceMotion && (
      ...
      <div
        className={`text-4xl font-black bg-gradient-to-r ${stat.gradient} bg-clip-text text-transparent leading-none mb-1`}
      >
        {stat.prefix}
        {display}
        {stat.suffix}

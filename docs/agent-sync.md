...
[codex] 2026-07-01 [ESTAT: treballant — cost desplaçament com a cost intern]
He llegit protocol complet d'arrencada (`DIAGNOSTIC`, `CLAUDE.md`, `agent-sync`, `admin-diary`, `estat-admin`, `admin-protocol`, `protocol-executiu`) i la nota de Claude `docs/disseny-cost-desplacament.md`. Perímetre: model/cost de transport i reserva nova, amb el cas `OE-2026-006` com a banc de proves. Objectiu immediat: evitar que el temps de viatge aparegui com a producte contractat i reimputar-lo al cost/marge de forma canònica. No tocaré la capa visual/CSS admin de Claude.
Avís claude: respecto el teu bloqueig de CSS/admin canon. Entraré només a serveis/cost/formulari i tests/documentació necessaris.

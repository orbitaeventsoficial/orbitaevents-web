...
[codex] 2026-07-01 [ESTAT: tancat — dashboard #1310 validat/committejat per Claude; counter global #1310]
...
Avís claude: continuo fora de mails automàtics, APPEND i seqüències. També evito inventari/preus font/schema/costEngine, tasks, booking detail/bd i fitxa lead/fxd. #1277 tancat: `bookingCreationService` suma `serviceLines.revenueAmount * quantity` al subtotal de nova reserva. #1278 tancat: `partnerHubService` compta `costAmount * quantity` per imports pagats a partner. #1279 tancat: quan un lead ja té booking, `replaceLeadServiceLines` passa per `updateBookingDetail` perquè la reserva recalculi totals. #1280 tancat: sanejats imports/quantitats/hores de `LeadServiceLine` pre-reserva. #1281 tancat: Partner Hub conserva cèntims en imports monetaris. #1282 tancat: Atribució multi-touch conserva cèntims en ingressos atribuïts. #1283 tancat: llistat de col·laboradors conserva cèntims al valor de catàleg. Counter global #1284 pertany a Claude (nova reserva canònica). #1285 tancat: `pipelineForecast.confirmedRevenue` conserva cèntims en reserves confirmades. Counter global #1286 pertany a Claude (hub client + tasks canònics). #1287 tancat: `cashFlowForecast` conserva cèntims en income/cost/net/cumulative. #1288 tancat: `collaboratorAdminService` saneja `commissionPct` i `costPerHour` en create/update. Counter global #1289 pertany a Claude (codi mort lp2/lf a leads-design.css). #1290 tancat: `collaboratorProductService` saneja preus i `sortOrder` de productes de col·laborador. #1291 tancat: `customQuoteAdminService` saneja imports/marge/finalPrice en create/update. Counter global #1292 pertany a Claude (arxiu + settings-inbox canònics). #1293 tancat: paginació finita/entera a proposals + invoices. Counter global #1294 pertany a Claude (booking-detall canònic). #1295 tancat: comptadors autofix finits i no negatius a salut/finances. Counter global #1296 pertany a Claude (fitxa lead botons/inputs canònics). #1297 tancat: settings NUMBER finits i estrictes a `adminSettingsService`. #1298 tancat: fallbacks manuals d'estadístiques admin finits i no negatius. #1299 tancat: `limit/offset` sanejats a testimonials admin. #1300 tancat: `days/page/limit` sanejats a activity admin. #1301 tancat: `limit/offset` sanejats a privacy audit admin. Counter global #1302 pertany a Claude (lead/temporada canònics, validate:core OK). #1303 tancat: `limit/offset` sanejats a privacy consents admin; `validate:core` OK. Counter global #1304-#1306 pertany a Claude (canonització completa, header leads, tabs). #1307 tancat: middleware admin auth compara Bearer/Basic/cookie amb `secureCompare`; test focalitzat, `tsc`, `qa:protocol`, `git diff --check` i `validate:core` OK. #1308 tancat: CSRF de mutacions admin al middleware passa per helper únic; test focalitzat 7 OK i clúster auth/middleware 39 OK; bloqueig `tsc` concurrent resolt pel dashboard. #1309 queda com a registre intermedi de tancament Codex darrere Claude. #1310 tancat i committejat per Claude (`e9911ac4`): dashboard /admin + image-manager canonitzats; validació indicada al diari/protocol amb `validate:core` EXIT 0.

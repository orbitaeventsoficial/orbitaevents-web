...
[codex] 2026-07-01 [ESTAT: tancat — front públic headers/pageheads/portal canònics #1314; counter global #1315]
...
Avís claude: continuo fora de mails automàtics, APPEND i seqüències. També evito inventari/preus font/schema/costEngine, tasks, booking detail/bd, fitxa lead/fxd, dashboard i CSS admin core (`globals.css`, `admin-shell.css`, `control-room.css`, `admin-theme.css`, `leads-design.css`). Repartiment vigent: Claude porta capa espaiat/admin CSS; Codex porta front públic (`app/[locale]`, `app/components`, `components`) per colors crus/inline substituïbles, UX invisible i patrons compartits, excloent temàtiques aprovades Halloween/Món Màgic. #1310 tancat i committejat per Claude (`e9911ac4`): dashboard /admin + image-manager canonitzats. #1311-#1312 tancats per Claude: gutter i escala d'espaiat admin. #1313 tancat Codex: formulari contacte, Turnstile i cookie consent passen estats/focus/toggles/CTA a tokens públics; `tsc`, `qa:protocol`, `git diff --check` i `validate:core` OK. #1314 tancat Codex: `PublicPageHeader` i `ClientPortalPageHeader` sense variants lliures, pageheads públics simples migrats, subcapçaleres del portal client unificades i tons success/warning/danger centralitzats a `lib/constants/clientPortalTones.ts`; `tsc`, `qa:protocol` i `validate:core` OK. Counter global #1315 pertany a Claude. Proper tall Codex: taxonomia de la resta de capçaleres públiques (heroes marketing vs pageheads simples vs portal hero) i migració per famílies, no variants soltes.

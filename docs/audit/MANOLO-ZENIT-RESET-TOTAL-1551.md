...
- **#1947**: `/admin/post-event`, `/admin/post-event/reports` i `/admin/post-event/feedback` deixen de fer cues locals de bookings `COMPLETED`; els pendents d'informe, enquesta i feedback passen per builders de `postEventPendingService` i `POST_EVENT_WORKFLOW.pendingTake`.

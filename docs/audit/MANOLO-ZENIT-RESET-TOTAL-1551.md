...
| Copilot Avui: connexio amb ranking transversal | EN CURS | #1558-#1562, #1577, #1580, #1583, #1617, #1618, #1619, #1620, #1623, #1844, #1845 | `/admin` ja consumeix NBA, degrada millor, mostra domini/finestra, evita lectura duplicada de capacity, pot pujar post-event, riscos economics dels proxims bolos, dossiers pendents i contractes acceptats pendents a la cua principal, omple buits amb `dailyBrief` sense duplicar el mateix desti, no perd caixa pendent dins la finestra operativa de 7 dies, calcula cobraments NBA/dailyBrief amb efectiu registrat i evita avisos del proper bolo si `outstandingAmount` ja és zero. |
| Roadmap nocturn visible | EN CURS | #1561 | Aquest bloc fa visible la columna vertebral de la nit. |
| Sistema / guards monocapa | EN CURS | #1575, #1610, #1624, #1686, #1719, #1720, #1721, #1722, #1723, #1724, #1930, #1929, #1939, #2011 | El roadmap ja esta protegit per guard propi, els catalegs declaratius detectats per `arch:layer:check` tornen a `lib/constants/*` sense trencar imports existents, el desglossament de caixa pendent viu en una sola font a `payment-status`, el cataleg de scripts ja no fa veure que una comanda s'ha copiat si el porta-retalls falla, Railway no mutila rutes Next per ignores massa amplis, la instrumentacio no deixa imports `@/lib` opacs al runtime de produccio, les notificacions finals del cron comercial no tomben tota l'automatitzacio si un canal extern fa timeout, el contracte de desat de leads queda protegit perquè una data canviada no pugui quedar atrapada en estat local/cache de temporada, els productes de partner separen visibilitat comercial de dossier i visibilitat interna de booking, els hard-delete de col·laboradors/productes queden bloquejats si hi ha dependències o si la BD no les pot verificar, els artefactes de prova Zenit comparteixen font única de marcadors i surten a `zenit:db:audit` com a handoff read-only, el fòssil `Task.legacyLeadTaskId` queda auditat a BD amb `legacyLeadTaskLinks=0` abans de qualsevol neteja schema, i `qa:patches` tolera fitxers efímers que desapareixen entre recollida i lectura sense relaxar detectors reals, i el Manual de màrqueting valida xarxa personal amb resposta/senyal real en comptes de feedback genèric. |
...
| Post-event i recurrencia | EN CURS | #1564, #1567, #1570, #1573, #1576, #1579, #1582, #1585, #1588, #1594, #1602, #1661, #1683, #1684, #1685, #1702, #1703, #1928, #1940, #1943, #1947, #1971, #1972, #1973, #1975, #1976, #1977, #1978, #1979, #1980, #1981, #1982, #1983, #1984, #1985, #1986, #1987, #1988, #1989, #1990, #1991, #1992, #1993, #1994, #1995, #1996, #1997, #1998, #1999, #2000, #2001, #2002, #2003, #2004, #2005, #2006, #2007, #2008, #2009, #2010 | "Tanca el cercle" i el playbook ja comparteixen desti; el playbook prepara accio segura, registra decisio comercial, avança quan el testimoni ja està sol.licitat, distingeix social preparat de publicat, crea esborrany social intern, l'obre directament a l'editor, bloqueja publicacio sense revisar consentiment, permet marcar revisio feta amb nota persistent, aplica la mateixa regla al servei server-side, l'email post-event de fitxa anuncia errors abans que l'operador assumeixi que ha sortit, les còpies de missatges de referral/reactivació/reengagement ja no fallen en silenci, les mutacions socials de publicar/arxivar/eliminar conserven el motiu backend, el catàleg de plantilles email no confon fallada de lectura amb catàleg buit, l'auditoria Zenit detecta bookings completats recents on el volant post-event no ha arrencat, la finestra de catch-up post-event viu en una font única compartida per emails, dashboard, brief, checklist, automació i auditoria, el playbook també consumeix la mateixa finestra/límit/cadència d'accions sense `90` o `3` locals, el hub/reports/feedback ja no tenen cues pròpies fora de `postEventPendingService`, el cron manual/automàtic de post-event comparteix una sola identitat operativa `automation.postEvent` amb fallback legacy només per historial vell, l'enviament manual de Home/Emails i fitxa individual exigeix doble confirmació abans de disparar emails reals, el playbook aterra l'agraïment directament al bloc `#sec-post-event` on viuen email, informe i retorn al playbook, la fitxa explica que l'enquesta pendent depèn de l'email post-event canònic, l'estat d'aquest email a admin i portal client el governa `Booking.postEventEmailSent/postEventEmailSentAt`, el tancament intern ja no usa `ClientFeedback` com a progrés o completat, el resum de timeline del portal distingeix informe completat, email enviat i email pendent, el detall de reserva ja no carrega `ClientFeedback` com a dependència residual, el hub post-event mostra emails pendents amb CTA segur cap a Emails, Dashboard/Control/QuickActions entren pel hub post-event en comptes de saltar directament a Emails, totes les subpantalles aterren al bloc `#sec-post-event`, Atles/Master Atlas ja documenten que la font d'estat és `Booking.postEventEmailSent`, no `ClientFeedback`, la tesi viva queda incorporada al comandament com a font documental alineada, el snapshot tècnic de lead ja no exporta `ClientFeedback` com a senyal post-event, el portal client ja no conserva claus internes `feedbackSent`/`pendingClose` per pintar l'email post-event, el Master Atlas ja marca l'auto-esborrany social revisable com a fet amb `SocialPost` DRAFT i guard de revisió, el Master Atlas ja reconeix que el report completat pot crear/reutilitzar `PortfolioEvent` DRAFT des de `/admin/post-event/reports` amb media marcada i revisió a `/admin/portfolio`, el hub/subpantalla de seguiment ja no presenten `Feedback al Client` com a etiqueta visible de producte, la subpantalla d'enquestes parla de valoracions, NPS i testimonis en comptes de feedback genèric, el brief diari demana review/testimoni en comptes de feedback, Sales Ops parla d'aprenentatge de valoracions/testimonis per millorar proposta i oferta, la tasca automàtica `POST_EVENT` demana review/testimoni en comptes de feedback, l'email post-event EN del servei canònic diu `Your review helps us improve`, les plantilles EN fallback `post_event`/`testimonial_reminder` demanen review en comptes de feedback, l'email EN de testimoni aprovat diu `We love reading your review`, el següent pas CRM post-event parla de review/testimoni, i el Repo Atlas defineix post-event com ressenya/testimoni/referral/recurrencia sense feedback visible, i la campanya CRM recent usa tipus/id de review en comptes de feedback, i la copy pública de survey/testimoni agraeix opinió en comptes de feedback, i els inventaris vius descriuen la ruta legacy com seguiment/agraïment en comptes de feedback. |
| Post-event i recurrencia - sync ruta seguiment | FET | #2013, #2014, #2015, #2016 | La cua viva usa `buildPendingPostEventFollowUpBookingWhere`, `/admin/post-event/seguiment` és la ruta canònica i `/admin/post-event/feedback` queda només com a alias legacy sense exportar helper `Feedback`. |
| Documents/PDF | EN CURS | #1565, #1568, #1571, #1574, #1581, #1584, #1587, #1590, #1593, #1596, #1598, #1601, #1604, #1605, #1636, #1691, #1692, #1694, #1695, #1696, #1708, #1716, #1717, #1718, #1723, #1724, #1927, #1933, #1936, #1937 | El dossier complet i el cicle pressupost/contracte deixen traça; contracte i pressupost enviat congelen snapshots v1, el Customer Hub els fa visibles, la timeline diferencia "obrir pressupost" de "obrir contracte", el PDF signat queda accionable també com a fita de negoci i cada document mostra ruta d'origen client/entrada/reserva també a la cronologia, al llistat de dossiers i a les traces de dossier enviat/PDF compost; les traces de dossier ja no semblen activitat generica, la timeline pot aillar documents sense duplicar panells, el PDF de factura pinta bestreta/resta amb el mateix pendent real cash-aware que la resta de la maquina, el generador de dossiers no crea lead/client nou si no pot verificar clients existents, avisa si no pot importar productes del lead i tampoc confon la cerca de leads fallida amb cap resultat, la reassignacio de pressupostos no confon una cerca de vincles fallida amb cap resultat, el Studio de pressupostos avisa quan no pot llegir clients del CRM, el llistat de dossiers no confirma enviaments, paperera, restauracio o purga si el backend ho rebutja, intake sintetitza WhatsApps llargs sense aixafar ni duplicar notes abans de crear dossier/lead, el cataleg del generador separa proveidor -> audiencia mentre accepta productes actius nous sense whitelist i torna a pintar imatges de producte quan existeixen, Bingo Musical KIDS entra com a producte infantil ofertable amb PVP 200, Partners no cau si un KPI de bolos passats apunta a una relacio absent de produccio, les columnes buides d'audiència desapareixen del generador i els snapshots antics poden recuperar imatge viva sense perdre la foto comercial congelada, l'auditoria Zenit de BD vigila que contractes avançats, factures actives i albarans avançats no perdin l'artefacte durable `pdfUrl/pdfKey`, els emails de pressupost queden traçats com a `proposal`, el `quoteSnapshot` de cada pressupost enviat queda auditat i el cos del correu ha de contenir el PDF del mateix `Proposal` perquè el que es previsualitza/desa/envia tingui una identitat documental única. |
...
| #1727, #1754 | Portfolio | `/admin/portfolio` deixa de marcar zero fals, exposa la pestanya `Imatges`, permet drop-in per categoria amb pipeline canònic de carpeta/nom/optimització i blinda que la portada d'event vingui de media de portfolio, galeria de booking o catàleg estàtic de portfolio, no de productes/partners. |
| #1728-#1731, #1733, #1738, #1742-#1749, #1755-#1758 | Lead / economia / repartiment | El lead veu el repartiment abans de reserva, però el transforma en cost col·laborador llegible: Masquerade cobra servei, ruta i compensacions segons tarifa Òrbita sense veure el net d'Òrbita com a peça principal; #1742 compacta la fitxa i fixa la ruta com `km - 50`, persones amb 1 h inclosa i dietes només si passa de 150 km; #1743 fa que la liquidació oculta de ruta viatgi a la reserva i que els peatges entrin al càrrec visible; #1744 impedeix que el dossier esborri aquesta liquidació en sincronitzar productes cap al lead; #1745 talla el soroll amb ruta agregada, cost col·laborador curt, rail de marge de decisio i detall complet reservat a reserva/economia; #1746 fa que `Crear dossier` creï/obri directament el PDF des del lead quan les dades ja hi són; #1747 redueix el repartiment a servei + ruta agregada - compensació i deixa la liquidació completa per reserva/economia; #1748 fa que lead i generador de dossiers creïn pel mateix contracte canònic `{ leadId }`; #1749 retira l'endpoint vell, plega el col·laborador i converteix el lead en pantalla de decisio amb dossier com accio principal; #1755 elimina la validació persistent amb partner i blinda Manolo perquè no pugui introduir schema/API/BD sense autorització explícita; #1756 deixa la pàgina capturada desktop/mòbil sense overflow, sense `a operatiu`, sense buit abans de l'històric i amb `Crear dossier` primari; #1757 repensa la distribució visual perquè client total, transport i cost col·laborador es llegeixin sense caixes gegants ni llenguatge de pacte pendent; #1758 integra peatges dins `Transport`, pinta el net en vigilància i centra el panell `Següent pas` amb vora clara. |
...
- **#1754**: `PortfolioEvent.coverImage` queda blindat perquè la portada d'event surti de media de portfolio, galeria de booking o catàleg estàtic de portfolio; les imatges de producte/partner continuen sent per dossier/producte i snapshots ja enviats.
- **#1755**: la `validació amb partner` surt del lead: el pacte torna a ser lectura comercial del repartiment, `Crear dossier` torna a ser acció principal i `qa:manolo-boundary` blinda que Manolo no autoritzi schema/migracions/endpoints/API/BD sense permís explícit del propietari.
- **#1756**: la pàgina del lead Alba queda verificada amb captures desktop/mòbil v4: nav mòbil sense retalls, Zenit sense overflow, `a operatiu` substituït per `inclòs`, històric sense espai mort i `Crear dossier` com a CTA principal.
- **#1757**: la mateixa pàgina passa per repàs de distribució: el resum client és fórmula compacta (`Serveis + Transport = Total client`), `Pacte/Tarifa partner` passa a `Cost col·laborador` i la caixa de Masquerade deixa de ser un contenidor gegant buit.
- **#1758**: el remat Manolo integra els peatges dins de `Transport`, pinta el `Net estimat` en to de vigilància i centra `Següent pas` com a panell més baixet, amb amplada conservada i vora daurada visible.
- **#1840**: documents comercials sense IVA ambigu, pressupostos amb ruta heretada del lead i Bingo +70 amb assistent operatiu comptant al transport.
- **#1841**: guàrdia funcional del payload final lead/dossier -> reserva: serveis visibles, cost col·laborador, tècnic inclòs, línia oculta `[travel-cost]`, km, peatges, transport i assistent Bingo +70 arriben junts sense filtrar `travelHeadcount`.
- **#1842**: `/admin/portfolio` deixa de crear events amb una URL manual de portada; ara tria una imatge existent de media de portfolio o catàleg estàtic segur, i els vídeos ja no s'ofereixen com a portada.
- **#1843**: dashboard econòmic blindat contra bolos antics amb `travelCost` + `[travel-cost]`: el marge visible suma el transport una sola vegada i no duplica imports interns de repartiment.
- **#1844**: `/admin` projecta la cua canònica de dossiers pendents dins `Fes això ara` com a accions `Document` cap a `/admin/dossiers?leadId=...`, sense crear procés documental paral·lel.
- **#1845**: `/admin` projecta contractes pendents de propostes acceptades (`generar`, `enviar`, `seguir signatura`) i corregeix el Customer Hub perquè `DRAFT` sense referència sigui `Generar contracte`, no `Enviar`.
- **#1846**: `/admin` projecta pressupostos `DRAFT` amb client assignat dins `Fes això ara` com a accions `Pressupost` cap a l'Studio canònic `/admin/presupuestos?customerId=...&proposalId=...`, i el trigger d'acceptació deixa de prometre contractes auto-generats quan només els deixa pendents.
- **#1927**: `zenit:db:audit` amplia la vigilància documental final: contractes `SENT|SIGNED`, factures actives i albarans `DELIVERED|SIGNED` han de conservar PDF/enllaç durable, amb guard a `qa:zenit-tooling` i auditoria real a 0 findings.
...
- **#1928**: `zenit:db:audit` detecta bookings `COMPLETED` recents sense email post-event, informe ni enquesta, i incoherències `postEventEmailSent=true` sense timestamp; la primera auditoria troba `OE-2026-004` i `OE-2026-005` com a post-event no arrencat.
- **#1929**: `zenit:db:audit` audita el camp temporal `Task.legacyLeadTaskId` i retorna `legacyLeadTaskLinks=0`, deixant la possible neteja schema com a tall separat basat en evidència.
- **#1930**: `zenit:db:audit` audita artefactes de prova Zenit amb la font única `lib/constants/adminTestArtifacts.json`; la primera lectura troba `adminTestArtifactLeads=14` i deixa el handoff a Claude sense cap eliminació física.
- **#1933**: el dispatch canònic de `Proposal` envia i registra el correu amb `orbitaKind=proposal`/`orbitaId=proposalId`, la Safata obre el fil com a `Pressupost`, les respostes preserven el context i `zenit:db:audit` afegeix `proposalEmailTraceMismatches` (primera lectura: 0) perquè cap email `proposal-send` pugui quedar sense identitat documental de primera classe.
- **#1936**: `zenit:db:audit` afegeix `sentLikeProposalsMissingQuoteSnapshot` i el guard `qa:zenit-tooling` exigeix `QUOTE_SNAPSHOT_V1`, de manera que cap `Proposal` `SENT|VIEWED` amb PDF pugui perdre o divergir de `snapshot.quoteSnapshot.documentType/proposalId/reference/pricing.total`.
- **#1937**: `zenit:db:audit` afegeix `proposalEmailDocumentMismatches`: cada `EmailSend` `proposal-send` amb `orbitaKind=proposal` ha de resoldre a un `Proposal` existent i contenir al subject/cos la referència i el PDF del mateix `Proposal`.
- **#1939**: `qa:patches` comprova existència i captura només `ENOENT` en lectura, de manera que un fitxer temporal desaparegut no atura `validate:core` mentre els detectors de patch-smell continuen actius.
- **#1940**: `lib/constants/postEventWorkflow.json` passa a ser la font única de finestra post-event (`2/3/90` dies) i `postEventPendingService` alimenta emails pendents, `/admin/emails`, dashboard, brief, checklist, automació i `zenit:db:audit` perquè `OE-2026-004` i `OE-2026-005` no desapareguin del catch-up operatiu.
- **#1943**: el playbook post-event entra a la mateixa font única: `postEventWorkflow.json` defineix `playbookTake` i `actionDueDays`, `postEventPlaybookService` consumeix `getPostEventWorkflowDates`, `POST_EVENT_DAY_MS` i `POST_EVENT_WORKFLOW`, i `/admin/post-event/playbook` mostra la finestra amb `catchupWindowDays` sense hardcodejar `90 dies`.
- **#1947**: `/admin/post-event`, `/admin/post-event/reports` i `/admin/post-event/feedback` deixen de fer cues locals de bookings `COMPLETED`; els pendents d'informe, enquesta i seguiment passen per builders de `postEventPendingService` i `POST_EVENT_WORKFLOW.pendingTake` (nom tècnic de seguiment alineat a #2013).
- **#1971**: el cron post-event manual i automàtic deixa de bifurcar estat entre `emails.cron` i `automation.postEvent`; `ADMIN_POST_EVENT_CRON_STATUS_PREFIX` governa escriptura i lectura a Emails, Home, Salut, Integracions i Crons, amb fallback legacy només per historial vell.
- **#1972**: els botons manuals d'enviament post-event a Home i Emails passen a confirmació de dos passos i copy d'emails reals, evitant que un clic accidental dispari comunicacions externes.
- **#1973**: el botó individual `PostEventEmailButton` de la fitxa de reserva també exigeix segon clic abans d'enviar un email real al client.
- **#1975**: el playbook post-event deixa d'aterrar a dades genèriques de client i obre `#sec-post-event`; la fitxa concentra email, informe intern i tornada al playbook dins el bloc post-event.
- **#1976**: la fitxa de reserva deixa de presentar l'enquesta com una acció separada amagada i mostra si s'enviarà amb l'email post-event o si ja està pendent de resposta.
- **#1977**: la targeta `Email post-event` de la fitxa consumeix `Booking.postEventEmailSent/postEventEmailSentAt`, la mateixa font que escriu el dispatch, i deixa de dependre de `ClientFeedback`.
- **#1978**: el portal client mostra l'estat post-event amb `Booking.postEventEmailSent` i copy públic d'email post-event enviat/pendent, sense dependre de `ClientFeedback`.
- **#1979**: `bookingOperationalService` treu `ClientFeedback.sentAt` de `internalPostEventStatus`; la fitxa mostra `Tancament intern` governat per informe, email post-event canònic i resposta real del client.
- **#1980**: el resum de timeline del portal substitueix `postEventReport ? done : progress` per una lectura canònica d'informe completat, email post-event enviat o email post-event pendent.
- **#1981**: la fitxa i `getBookingDetail()` deixen de carregar `clientFeedback: true` quan el detall de reserva ja no l'usa per cap estat post-event.
- **#1982**: `/admin/post-event` consumeix `buildPendingPostEventEmailBookingWhere` i mostra `Emails pendents` amb CTA segur a `/admin/emails`.
- **#1983**: el dashboard canvia l'alerta `Emails post-event pendents` perquè entri a `/admin/post-event` i no salti directament a `/admin/emails`.
- **#1984**: el pilot de `/admin/control` canvia `ADMIN_DASHBOARD_PILOT_STEPS.postevent.href` a `/admin/post-event`.
- **#1985**: `/admin/emails` reutilitza el botó canònic d'email post-event amb doble confirmació, cada pendent pot obrir `#sec-post-event` de la reserva, i el badge manual `Comença per pas 3` de `/admin/control` deixa de saltar directament a Emails.
- **#1986**: `feedback`, `reports`, `surveys` i el link de client del `playbook` post-event obren la reserva directament a `#sec-post-event`, i Feedback consumeix el component compartit d'email post-event.
- **#1987**: el helper del playbook porta el testimoni sense `customerId` al bloc `#sec-post-event` de la reserva, no a `#sec-client`.
- **#1988**: `QuickActions` del dashboard deixa d'executar `/api/admin/emails/run-cron` i obre `/admin/post-event`, mantenint els enviaments reals dins Emails amb confirmació.
- **#1989**: l'atles elèctric deixa de presentar `ClientFeedback` com a `sourceOfTruth` post-event i apunta a playbook, cues, dispatch, `Booking.postEventEmailSent`, informes, enquestes i testimonis.
- **#1990**: el Master Atlas incorpora `/admin/post-event/feedback`, `postEventPendingService`, `postEventDispatchService`, `Booking.postEventEmailSent` i `CustomerTestimonial`, i explicita que `ClientFeedback` no és estat operatiu.
- **#1991**: aquest full de ruta sincronitza la fila executiva i el detall Post-event fins al #1990 perquè el comandament Manolo no quedi per darrere del protocol viu.
- **#1992**: la tesi viva de la màquina deixa de descriure el cervell post-event amb `ClientFeedback` i el substitueix per `postEventPendingService`, `postEventDispatchService`, `Booking.postEventEmailSent`, `ClientSurvey` i `CustomerTestimonial`.
- **#1993**: aquest full de ruta incorpora la tesi #1992 al front `Post-event i recurrencia` perquè roadmap i protocol no tornin a divergir.
- **#1994**: `leadSnapshotService` deixa de seleccionar `clientFeedback` i de publicar `hasClientFeedback` dins `stats.postEvent`; el snapshot tècnic queda en email, informe, enquesta i review/testimoni.
- **#1995**: el portal client deixa de pintar l'estat post-event amb claus internes `feedbackSent`/`pendingClose`; el contracte i els tests parlen d'email post-event canònic sense canviar la copy pública.
- **#1996**: el Master Atlas reconeix l'auto-esborrany social revisable com a `FET`: `postEventRecurrenceDecisionService` crea/reutilitza `SocialPost` DRAFT i `socialPostReviewGuard` impedeix publicar sense revisió.
- **#1997**: el Master Atlas reconeix `Portfolio/testimoni des de bolo completat` com a `FET`: `/admin/post-event/reports` ofereix `EnsurePortfolioEventButton`, l'API `/api/admin/post-event/portfolio-event` passa per CSRF/auth i `portfolioEventService` crea/reutilitza `PortfolioEvent` DRAFT només amb informe completat i foto marcada.
- **#1998**: el hub post-event i la subpantalla `/admin/post-event/feedback` deixen de presentar `Feedback al Client`/`Veure feedback` com a etiqueta visible; el flux queda com `Agraïment al Client` i `seguiment`, mantenint la ruta tècnica i el botó d'email canònic.
- **#1999**: `/admin/post-event/surveys` deixa el subtítol visible `Feedback i valoracions dels clients` i passa a `Valoracions, NPS i testimonis dels clients`, mantenint `ClientSurvey` i l'anchor post-event.
- **#2000**: el brief diari de post-event deixa de dir `demana feedback` i passa a `demana review o testimoni`, mantenint l'alerta cap a `/admin/post-event`.
- **#2001**: `/admin/sales-ops` deixa `Bucle de feedback`/`feedback incorporat` i passa a aprenentatge de valoracions i testimonis dins l'auditoria i el pla d'escalat.
- **#2002**: la proposta automàtica `POST_EVENT` de `taskAutomationService` deixa de demanar feedback i passa a demanar review o testimoni abans de tancar el cicle.
- **#2003**: l'email post-event EN generat per `postEventEmailService` deixa `Your feedback helps us improve` i passa a `Your review helps us improve`.
- **#2004**: les plantilles EN fallback `post_event` i `testimonial_reminder` de `emailTemplateService` deixen `feedback` i passen a review dins subject, body i preheader.
- **#2005**: `TESTIMONIAL_COPY.en.approved` deixa `We love reading your feedback` i passa a `We love reading your review`, amb test i18n directe.
- **#2006**: `CUSTOMER_NEXT_STEPS.POST_EVENT.hint` deixa `Tanca feedback...` i passa a `Tanca review, testimoni i seguiment de recurrencia.`
- **#2007**: el diccionari `post-event` del Repo Atlas deixa de definir el volant amb `feedback` visible i passa a ressenya, testimoni, referral i recurrencia.
- **#2008**: la campanya CRM recent deixa el tipus/id `FEEDBACK_REQUEST`/`campaign:feedback-recent` i passa a `REVIEW_REQUEST`/`campaign:review-recent`.
- **#2009**: la copy pública de survey/testimoni en CA/ES/EN demana i agraeix opinió, sense `feedback` visible al CTA EN ni als missatges gamificats ES/EN.
- **#2010**: `estat-admin`, inventari Frankenstein i fitxes de pantalles descriuen `/admin/post-event/feedback` com seguiment/agraïment, no com etiqueta de producte feedback.
- **#2011**: el gate de màrqueting del Manual canvia `feedback immediat` per resposta/senyal real i deixa de demanar `recollir feedback`.
- **#2013**: la cua de seguiment consumeix `buildPendingPostEventFollowUpBookingWhere`; `Feedback` queda només com alias tècnic de compatibilitat i `qa:zenit-tooling` exigeix el nom de seguiment.
- **#2014**: `/admin/post-event/seguiment` passa a ser la ruta canònica del seguiment post-event; el hub hi navega, Master Atlas la reconeix i `/admin/post-event/feedback` queda com a alias legacy.
- **#2015**: `buildPendingPostEventFeedbackBookingWhere` queda retirat; el servei, el guard i les fixtures només exposen el builder `FollowUp` per a la cua de seguiment.
- **#2016**: la fila executiva del front `Post-event i recurrencia` queda sincronitzada amb #2013-#2015: ruta canònica `/admin/post-event/seguiment`, alias legacy `/admin/post-event/feedback` i helper viu `buildPendingPostEventFollowUpBookingWhere`.
...
1. **E2E lead -> dossier -> reserva.** Reproduir un lead real amb productes, desplaçament, peatges, dietes, imatges i cost col·laborador; #1733 blinda `hours`/`partyType`, #1734 blinda km/peatges en seleccionar lead dins del generador, #1735 blinda peatges en snapshots antics, #1736 alinea PDF compost amb email/HTML, #1738 explica el repartiment del col·laborador, #1739 arregla previews blob amb imatges, #1740 ordena jerarquia/imatges responsives, #1741 alinea vehicle client amb liquidació de ruta llarga, #1743 conserva la liquidació oculta de ruta en crear reserva, #1744 evita que el dossier l'esborri en sincronitzar productes, #1745 força imatges completes al dossier, #1746 crea/obre el PDF directament des del lead, #1747 deixa el cost col·laborador en resum curt amb portada Bingo adulta viva, #1748 unifica el contracte de creació lead/generador, #1749 retira el cami redundant mentre deixa el lead en mode decisio, #1755 retira la validació amb partner perquè el repartiment no sigui un estat de negoci, #1756 deixa la pàgina visualment capturada sense overflow, #1757 compacta la distribució de client/transport/col·laborador, #1758 remata peatges/net/següent pas, #1840 incorpora IVA/transport/Bingo +70 als documents comercials i #1841 blinda el payload final de reserva amb serveis, `[travel-cost]`, km, peatges, transport i assistent. **Passada browser/captura real ja feta** (Canvis #1850-#1875 + verificació addicional 2026-07-11 sense codi nou, veure `docs/admin-diary.md`): `.codex-captures/zenit-e2e-1850/*` (28/28 checks + captures lead/dossier/booking), `.codex-captures/zenit-proposal-booking-1855/*` (proposta→reserva amb transport real coherent) i `.codex-captures/zenit-e2e-verify-1919/*` (passada en viu, PDF compost 200/coherent, 0 errors de pàgina). Aquest punt ja no és un pendent obert.
2. **Portfolio/media pipeline.** #1754 blinda la frontera d'origen de portades d'event (portfolio/bookings/static portfolio, no producte/partner) i #1842 elimina la URL manual del formulari perquè el nou event triï només una imatge existent de la categoria. Encara falta, si es vol rematar aquest front, una prova real browser/drop-in amb pujada/substitució i captura de l'admin.
3. **Economia de bolos antics.** #1737 blinda la duplicació latent entre `travelCost` i línies `[travel-cost]` dins el motor, i #1843 ho porta al dashboard econòmic perquè marge/cobrament visible no dupliqui ruta. Queda pendent només revisar casos reals si apareix una alerta incoherent, no migrar ruta sense evidència.
4. **Copilot de gestió.** #1844 porta dossiers pendents a `Avui`, #1845 hi porta contractes acceptats pendents i #1846 hi porta pressupostos `DRAFT` vius cap a l'Studio canònic; continuar convertint qualsevol fallada real nova de cobrament, marge, post-event o lead calent en acció executiva, no en pantalla amagada.
